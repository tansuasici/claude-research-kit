...
- **HTML Artifacts module** (TAN-3615) — `ARTIFACTS.md` conventions + `artifacts/design-system.html` (reference tokens, indigo brand) + `artifacts/index.html` (catalog). Default to HTML for shareable, read-only outputs — response-letters, submission checklists, review reports, results tables, literature maps — while the manuscript stays LaTeX. The cardinal rule holds: an artifact never invents a value. Wired into `CLAUDE.md`, `install.sh`, and doctor's Optional Modules.
- **Analytics & memory** (TAN-3616) — `/note` (across-compaction journal: finding/decision/summary → folded into the session handoff by `journal-fold.sh` at SessionEnd), `/scorecard` (per-session telemetry from `reports/session-audit.log`), `/retro` (windowed retrospective + recurring reviewer patterns), `/review-resurface` (dormant `tasks/reviews/` notes by topic — pointers only). Backed by `scripts/note.sh`, `scripts/review-resurface.sh`, and the `journal-fold` hook. **23 skills, 14 hooks**; bench → **34 scenarios**.
